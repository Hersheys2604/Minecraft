...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        """
        #Step 1: Account or base case if "subtree is a single leaf node"
        #Step 2: Go right once "smallest key AMONG THE LARGER KEYS"
        #Step 3: Keep going left until hit a leaf.
        if (current.left is None and current.right is None) or (current.left is not None and current.right is None): #Base case --> if no children OR only smaller children 
            return None 
        else: #We only proceed, if we can move down to a RIGHT node. As all left side nodes would be less than the current node. (condition required)
            current = current.right #Move right once (as "smallest key among all the larger keys")
            while current.left is not None: #Keep moving left until leaf has been hit.
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        """
        #Step 1: Keep going left until reach leaf.
        #while current.left is not None and current.right is not None:
        while current.left is not None:
            current = current.left 
        return current

    def get_maximum(self, current: TreeNode) -> TreeNode:
        """
