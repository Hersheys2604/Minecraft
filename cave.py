...
"""
This file contains the cave names and the Cave class where it contains the name of the cave, the material and the quantity of material
"""
...
class Cave:
    """
    Cave contains the name of the cave, the Material class and the quantity of the material in the cave.
    The quantity of the cave can be adjusted by the user accordingly. 
    """
    
    def __init__(self, name: str, material: Material, quantity: float=0) -> None:
        """
        Inititialises all variables of a Cave Instance
    ...
    def add_quantity(self, amount: float) -> None: 
        """
        Adds 'amount'  to the amount of material in the cave
        Best and Worst case Complexity: O(1)
        """
    ...
    def remove_quantity(self, amount: float) -> None:
        """
        Removes 'amount' from teh amount of material in the cave
        Best and Worst case Complexity: O(1)
        """
        self.quantity = self.quantity - amount
    ...
    def get_quantity(self) -> float:
        """
        Returns the quantity of material in a cave
        Best and Worst case Complexity: O(1)
        """
        return self.quantity
    
    def if_visited(self, food: Food) -> bool:
        """
        Checks if the cave has been visited previously. Part of the 'select_food_and_caves' method.
        Best and Worst case Complexity: O(1)
        """
        if food != self.food:
            self.food = food
    ...
    def __str__(self) -> str:  
        """
        String representation of a Cave Class
        Best and Worst case Complexity: O(1)
        """
        return f"Cave name: {self.name}, {self.material}, Quantity: {self.quantity}"

    @classmethod
    def random_cave(self, material_list: list[Material]) -> Cave:       
        """
        Returns a Random Cave Class.
        Best and Worst case Complexity: O(1)
        """
        return Cave(CAVE_NAMES[RandomGen.randint(0, len(CAVE_NAMES) - 1)], material_list[RandomGen.randint(0, len(material_list) - 1)], RandomGen.randint(0, 10))     
